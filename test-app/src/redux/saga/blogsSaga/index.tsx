...
function* fetchBlogsWithFilter({
  payload,
}: ReturnType<typeof blogsActionCreators.getBlogsWithFilter>) {
  yield put(blogsActionCreators.setBlogsFilter(payload));

  yield fetchBlogs();
}

function* fetchBlogsWithPage({
  payload,
}: ReturnType<typeof blogsActionCreators.getBlogsWithPage>) {
...
function* fetchPagesCount() {
  try {
    ...
    const response: AxiosResponse<IBlogsResponsePagesCount> = yield call(
      getPagesCount
    );
    if (response.data && response.status === 200) {
